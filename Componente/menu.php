<div class="sidenav">
        <a href="../index/form_inicial.php">Página Inicial</a>
        <a href="../aluno/form_aluno.php">Administração de alunos</a>
        <a href="../acesso/form_acesso.php">Administração de acessos</a>
        <a href="../Materia/form_materia.php">Administração de materias</a>
        <a href="../matricula/form_matricula.php">Administração de matriculas</a>
        <a href="../Avaliacao/form_avaliacao.php">Administração de avaliacões</a>
        <a href="../notas/form_notas.php">Administração de notas</a>
        <a href="../Utils/proc_sessiondestroy.php">Sair</a>
</div>